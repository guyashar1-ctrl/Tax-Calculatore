 ...
 * לגיל**. רק הסטטוסים ש-cancel_automation_job מקבל (queued/needs_human);
 * running אינה ניתנת לביטול, והעובד תופס אותה מחדש בעצמו כשהחכירה פגה.
 */
export function mustCancelBeforeStart(job: AutomationJob | null): boolean {
  return !!job && (job.status === 'needs_human' || job.status === 'queued');
}
...
export function derivePhase(input: DeriveInput): DerivedConnState {
  ...
  if (job) {
    ...
    if (job.status === 'needs_human') {
      ...
      if (ageMs <= CONNECTING_TIMEOUT_MS) return { ...idle, phase: 'connecting' };
