...
import TestBtlSync from './components/authorities/__TestBtlSync';
...
export default function App() {
  ...
  if (import.meta.env.DEV && typeof window !== 'undefined' && new URLSearchParams(window.location.search).has('test-btl-sync')) {
    return <TestBtlSync />;
  }
    ...
    <div className="app">
      <header className="header">
        ...
        {/* נוריות החיבור לרשויות — צמודות לניווט, כי הן מצב גלובלי של
            ...
            ‼ **לא** מאחורי checksTab: אלה פקדי מוצר, לא דיאגנוסטיקה של פיתוח.
            checksTab שמור אך ורק להרתך הבדיקות בכרטיס הלקוח. כשהעובד המקומי
            כבוי הפקד אומר זאת במפורש במקום להיעלם — "אין כפתור" הוא מצב שאי
            אפשר לאבחן ממנו כלום. */}
        <AuthorityConnectionButtons userId={user?.id} />
