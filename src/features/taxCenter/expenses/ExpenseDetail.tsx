  ...
  return (
    <div className="card">
      ...
      >
        ...
        <span style={{ color: 'var(--gray-400)', transform: open ? 'rotate(180deg)' : 'none', transition: 'transform .15s', fontSize: '13px' }}>▼</span>
      </div>
      {open && (
        <div className="card-body" style={{ borderTop: '1px solid var(--gray-100)', fontSize: '14px', lineHeight: 1.7 }}>
          {children}
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '.9rem' }}>
      ...
      <div className="card" style={{ borderRight: `4px solid ${risk.color}` }}>
        <div className="card-body">
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '.9rem', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '34px' }}>{topic.icon}</span>
            <div style={{ flex: 1, minWidth: 220 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem', flexWrap: 'wrap' }}>
                <h2 style={{ fontSize: '20px', fontWeight: 600, margin: 0 }}>{topic.title}</h2>
                <span style={{ fontSize: '12px', fontWeight: 600, color: risk.color }}>
                  {risk.icon} {risk.label}
                </span>
              </div>
              <p style={{ fontSize: '14px', color: 'var(--gray-600)', margin: '.35rem 0 0', lineHeight: 1.6 }}>
                {topic.summary}
              </p>
              {topic.riskNote && (
                <div style={{ fontSize: '12px', color: 'var(--gray-500)', marginTop: '.3rem' }}>{topic.riskNote}</div>
              )}
              {topic.confidence && (
                <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '.6rem', marginTop: '.9rem' }}>
            <div style={{ background: it.bg, borderRadius: 8, padding: '.6rem .8rem' }}>
              <div style={{ fontSize: '12px', color: 'var(--gray-500)' }}>מס הכנסה</div>
              <div style={{ fontWeight: 600, color: it.color }}>{it.label} · {topic.incomeTax.shortLabel}</div>
            </div>
            <div style={{ background: vat.bg, borderRadius: 8, padding: '.6rem .8rem' }}>
              <div style={{ fontSize: '12px', color: 'var(--gray-500)' }}>מע"מ (תשומות)</div>
              <div style={{ fontWeight: 600, color: vat.color }}>{vat.label} · {topic.vat.shortLabel}</div>
            </div>
            <div style={{ background: 'var(--gray-50)', borderRadius: 8, padding: '.6rem .8rem' }}>
              <div style={{ fontSize: '12px', color: 'var(--gray-500)' }}>מקור מרכזי</div>
              <div style={{ fontWeight: 600, fontSize: '14px' }}>{topic.mainSource}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ border: '2px solid var(--blue-border)' }}>
        ...
        <div className="card-body" style={{ fontSize: '15px', lineHeight: 1.8, fontWeight: 500 }}>
          {topic.clientAnswer}
        </div>
      </div>
      ...
      <Section icon="🧾" title='מע"מ — קיזוז תשומות' defaultOpen>
        {topic.vat.legalBasis && (
          <div style={{
            ...
            marginBottom: '.6rem', fontSize: '13px',
          }}>
            <span style={{ fontWeight: 600 }}>בסיס חוקי: </span>{topic.vat.legalBasis}
          </div>
        )}
        <Bullets items={topic.vat.detail} />
        {(topic.vat.exceptions?.length ?? 0) > 0 && (
          <div style={{ marginTop: '.6rem' }}>
            <div style={{ fontWeight: 600, fontSize: '13px', color: 'var(--chip-blue-tx)', marginBottom: '.25rem' }}>↪ חריגים</div>
            <Bullets items={topic.vat.exceptions!} />
          </div>
        )}
        {(topic.vat.mistakes?.length ?? 0) > 0 && (
          <div style={{ marginTop: '.6rem' }}>
            <div style={{ fontWeight: 600, fontSize: '13px', color: 'var(--err)', marginBottom: '.25rem' }}>טעויות קיזוז נפוצות</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '.5rem' }}>
          {topic.legalSources.map((s, i) => (
            <div key={i}>
              ...
              <div style={{ fontSize: '13px', color: 'var(--gray-600)' }}>{s.gist}</div>
            </div>
          ))}
        </div>
      </Section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '.75rem' }}>
            {topic.caseLaw.map((c, i) => (
              <div key={i} style={{ borderRight: '3px solid var(--info)', paddingRight: '.75rem' }}>
                ...
                <div style={{ fontSize: '13px', margin: '.25rem 0' }}>{c.summary}</div>
            ...
            {topic.circulars.map((c, i) => (
              <div key={i} style={{ border: '1px solid var(--gray-200)', borderRadius: 8, overflow: 'hidden' }}>
                <div style={{ background: 'var(--gray-50)', padding: '.5rem .8rem', display: 'flex', gap: '.5rem', alignItems: 'baseline', flexWrap: 'wrap' }}>
                  ...
                  {c.number && <span className="badge badge-blue" style={{ fontSize: '12px' }}>{c.number}</span>}
                </div>
                <div style={{ padding: '.5rem .8rem', fontSize: '13px', display: 'flex', flexDirection: 'column', gap: '.3rem' }}>
      ...
      {topic.warnings.length > 0 && (
        <div className="alert alert-warning" style={{ marginBottom: 0 }}>
          <div style={{ fontWeight: 600, marginBottom: '.35rem' }}>שים לב</div>
