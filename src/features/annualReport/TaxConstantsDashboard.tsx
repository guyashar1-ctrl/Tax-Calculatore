  ...
  return (
    <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
      <div className="card-body" style={{ flex: 1 }}>
        <div style={{ color: 'var(--gray-500)', fontSize: '14px', marginBottom: '.3rem' }}>{card.title}</div>
        <div style={{ fontSize: '24px', fontWeight: 600, color: 'var(--gray-800)' }}>{card.primary}</div>
        {card.detail && <div style={{ fontSize: '14px', color: 'var(--gray-600)', marginTop: '.3rem' }}>{card.detail}</div>}
        {card.legalReference && (
          <div style={{ fontSize: '12px', color: 'var(--blue)', marginTop: '.5rem' }}>{card.legalReference}</div>
        )}
      </div>
