...
export interface NiTracking {
  ...
  /**
   * איך ההוראות הגיעו למבוטח:
   *   signature  — נכללו במייל בקשת החתימה (הרצוי, מייל אחד לשתי הפעולות)
   *   standalone — מייל נפרד של הוראות אישור ב"ל
   *   link       — הקישור האישי הועתק ונמסר ביד (וואטסאפ/טלפון). ‼ זו ראיה
   *                **חלשה יותר**: יודעים שהקישור יצא מכאן, לא שהוא הגיע.
   *                נשמר בנפרד בדיוק כדי שהמסך יוכל לומר את זה במילים.
   */
  instructionsSentWith?: 'signature' | 'standalone' | 'link';
