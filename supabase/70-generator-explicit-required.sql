-- 70 · המחולל קובע חובה/רשות במפורש
--
-- ‼ לא קישוט. אחרי מיגרציה 68 ברירת המחדל של העמודה היא true, ולכן כל שלב
--   שהמחולל יוצר נולד "נדרש" — כולל שני שלבי הזנב הארוך שמעולם לא חסמו סגירה:
--   first_month_review (ביקורת חודש ראשון) ו-representation_upgrade (מעבר
--   מייצוג משני לראשי, שיכול להימשך חודשים). בלי המיגרציה הזאת כל לקוח חדש
--   היה נחסם מסגירת קליטה בגלל עבודה שממשיכה ברקע אחרי שהוא כבר שוטף.
--   התגלה בסביבת הבדיקות: לקוח דמה שנוצר מאישור הצעה קיבל first_month_review
--   עם נדרש=true.
--
-- ‼ שתי הפונקציות הן ההגדרות החיות (live-2026-08-07) עם שינוי אחד בלבד:
--   העמודה required_for_close נוספה לרשימת העמודות, וערך מפורש נוסף לערכים —
--   false לשני סוגי הזנב הארוך, true לכל השאר. שום התנהגות אחרת לא נגעה.
--   הקובץ נוצר בטרנספורמציה על טקסט ההגדרה החיה ולא הוקלד מחדש.
--
...
begin
  ...
  if v_needs_paperless then
    ...
    if v_id_invite is null then
      ...
      if not p_dry_run then
        insert into public.onboarding_steps (user_id, engagement_id, client_id, required_for_close, step_type, track, scope, status, ball, payload)
        values (e.user_id, e.id, e.client_id, true, 'paperless_invite', 'tools', 'person', 'pending', 'me',
    ...
    if v_id_conn is null then
      ...
      if not p_dry_run then
        insert into public.onboarding_steps (user_id, engagement_id, client_id, required_for_close, step_type, track, scope, status, ball, depends_on_step_id, payload)
        values (e.user_id, e.id, e.client_id, true, 'paperless_connection', 'tools', 'person', 'locked', 'client', v_id_invite,
                jsonb_build_object(
