...
-- ‼ ensure_institution_alignment_steps בודקת בעלות על הלקוח — אבל לא על
-- p_engagement_id, שנכתב לתוך השורות כמו שהוא. משמעות: אפשר להטביע בשלבים
-- **שלי** את מזהה ההתקשרות של מישהו אחר. השורות בלתי נראות לו (RLS), אבל
-- onboarding_close_readiness סופרת לפי engagement_id בלי סינון בעלים, ולכן
-- סגירת הקליטה שלו נחסמת לצמיתות ע"י שורות שהוא לא יכול לראות ולא למחוק.
-- ‼ התיקון מוסיף בדיקה אחת ואינו נוגע בשום התנהגות תקינה: כל הקוראים
-- הלגיטימיים מעבירים התקשרות של אותו לקוח.
...
begin
  ...
  -- ‼ השער החדש: ההתקשרות חייבת להיות של אותו בעלים ושל אותו לקוח.
