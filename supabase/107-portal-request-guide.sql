-- ─── 107 · ההסבר של הבקשה מגיע לדף האישי ────────────────────────────────────
-- build_client_portal: שלושה שדות תוכן נוספים על פריט של בקשה חופשית —
--   note      (payload.clientNote)      · הסבר מלא שנפתח עם הבקשה
--   refs      (payload.clientRefs)      · שורות "שם ומספר" להעתקה
--   noteAfter (payload.clientNoteAfter) · משפט הסגירה שלפני הפעולות
--
-- למה: clientSub מוחלף בשורת ההתקדמות ("1 מתוך 2 הושלמו") כשיש יותר מדרישה
-- אחת, ולכן הסבר שנשען עליו נעלם בדיוק בבקשות המורכבות. הצרכן הראשון הוא
-- "הקמת הרשאה לחיוב חשבון" (קודי המוסד של הרשויות), אבל השדות גנריים.
--
-- ‼ תוספת בלבד: jsonb_strip_nulls מסלק אותם כשהם חסרים, ולכן כל בקשה קיימת
-- מחזירה בדיוק את אותו jsonb כמו לפני השינוי. שאר הגוף לא נגע — הנוסח נלקח
-- מ-pg_get_functiondef החי (supabase/live-2026-08-16/build_client_portal.sql).
