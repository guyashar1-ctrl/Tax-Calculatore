-- ═══════════════════════════════════════════════════════════════════════════
--  26 — אישור הצעה ללקוח שכבר בייצוג: בלי לאבד רשויות ובלי מייל מטעה
-- ═══════════════════════════════════════════════════════════════════════════
--  מה קרה (2026-07-30, הצעה 2026-011):
--  ההצעה ביקשה ייצוג בארבע רשויות. ללקוח כבר היה תהליך ייצוג פתוח, ולכן השער
--  "לא לפתוח ייצוג שני לאותו לקוח" תפס — נכון כשלעצמו. אבל:
--    1. הרשויות שההצעה החדשה ביקשה נזרקו. "ניכויים" לא הגיע לא לכרטיס הלקוח
--       ולא לבקשה, בלי שום סימן.
--    2. ההצעה קיבלה את טוקן ההשלמה של הבקשה הישנה, וללקוח יצא מייל "נשאר
--       לאמת את הזהות" — לטופס שהוא כבר מילא.
--    3. לרו"ח לא היה שום חיווי. ההצעה נראתה כאילו הכול רץ.
--
--  התיקון: איחוד רשויות במקום דריסה, וטוקן השלמה מוחזר רק כשבאמת צריך למלא.
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if v_client_id is not null then
    ...
    -- ─── ללקוח כבר יש תהליך ייצוג ───
    -- לא פותחים שני, אבל כן מוסיפים את מה שההצעה החדשה ביקשה. הרשויות
    -- מאוחדות ולא נדרסות: לרשות שכבר פעילה יש סטטוס אמיתי (ואולי אסמכתא),
    -- ודריסה בערך "בתהליך" הייתה מוחקת אותו. לכן הקיים גובר.
...
-- ─── האישור מחזיר טוקן השלמה רק כשבאמת נשאר למלא ─────────────────────────
-- בקשה שכבר מולאה אינה צריכה עוד סבב זיהוי. החזרת הטוקן שלה גרמה לאפליקציה
-- לשלוח ללקוח "נשאר לאמת את הזהות" ולהעביר אותו לטופס שכבר מילא.
-- repReused מאפשר לאפליקציה להסביר לרו"ח למה לא נפתחה בקשה חדשה.
...
begin
  ...
  if q.status = 'approved' then
    ...
    -- הבקשה שהוחזרה נוצרה קודם ⇒ נעשה שימוש חוזר בתהליך קיים
