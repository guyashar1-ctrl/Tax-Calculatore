...
‼ **המספר 133 נתפס פעמיים**, בדיוק כמו 131 לפניו: שני סשנים מקבילים לקחו אותו
באותו יום. שתי המיגרציות רצו בפועל ושתיהן חיות —
`133-rep-approval-as-substep.sql` (הזירוז כצעד בתוך הייצוג) ו-
`133-hyphen-in-sql-functions.sql` (כאן). הן אינן נוגעות זו בזו: הראשונה בקוד
המסכים, השנייה בהגדרות הפונקציות. השמות ב-`schema_migrations` הם שמות הקבצים,
ולכן הם נשארו כפי שרצו ולא שוכתבו בדיעבד. **המספר הפנוי הבא הוא 135.**
...
כתבו בעבר נשאר עם "—". הכרעת גיא (2026-08-25): ליישר גם אותו, בשלוש עמודות —
`onboarding_events.note`, `tasks.title`, `tasks.description`.

**מה הוחל בפרודקשן:** 43 שורות — יומן 30, כותרות 11, גוף משימה 2.
ב-staging: 13 — יומן 12, כותרות 0, גוף משימה 1.

‼ **הנרמול חשף כפילות אמיתית של משימת מערכת.** האינדקס
...
יצרה עותק שני של "בדיקת עדכניות נתוני מרכז הידע Q3/2026". בפרודקשן: המקורי
(`b732c934`, 2026-07-21) כבר **בוצע**, והכפיל (`9d669a0f`, 2026-08-24) **פתוח**.

המיגרציה **אינה מוחקת את הכפילות** — זו הכרעה על נתון אמת. היא מדלגת על
השורה, מודיעה עליה בשם, והאימות מוודא שכל שורה שנשארה עם "—" חסומה מהסיבה
הזאת בדיוק ולא מסיבה אחרת. לכן נשארה **כותרת אחת** עם מקף ארוך בכל סביבה.

‼ **הלקח מעבר למקף:** כל שינוי בניסוח של כותרת משימת מערכת שובר את מפתח
הזהות שלה ויוצר עותק. מפתח היציבות היחיד היום הוא התג שבסוגריים
(`[בדיקת-עדכניות Q3/2026]`) — והאינדקס אינו מסתמך עליו.

‼ **מה לא נגעו בו:** `email_messages` (מה שהנמען קיבל בפועל);
`accountant_notifications` — שלוש השורות עם "—" כולן `suppressed`, לא נשלחו
ולא יישלחו; `documents` / `clients` / `quotations` — טקסט שגיא הקליד בעצמו,
מחוץ להיקף שהתבקש.

‼ `tasks.updated_at` זז ל-`now()` בשורות שנגענו בהן (טריגר `tasks_updated_at`).
נבדק שאין לזה משמעות במסך: `updatedAt` של משימה משמש רק כמפתח מיון משני
למשימות שהושלמו **ללא** `completedAt`, ולכל המשימות המושלמות שנגענו בהן יש
`completedAt`. הטריגר לא נוטרל בכוונה, והאימות מוודא שהוא פעיל.

---

## 144 · שליחת מסמכים ללקוח — כמה קבצים, שני מקורות, ומלל חופשי (2026-08-31)

`supabase/144-send-documents-multi-source.sql` · הוחלה על **staging ופרודקשן**.

עד כה בקשת «שליחת מסמך ללקוח» נשאה קובץ אחד, מספריית המשרד בלבד, ונסגרה
בהצהרת "עברתי על המדריך". שלוש ההגבלות נופלות:

| # | מה | איפה זה חי |
|---|---|---|
| 1 | כמה קבצים בבקשה אחת | `payload.clientResources[]` (רבים) במקום `clientResource` (יחיד) |
| 2 | גם קובץ מהתיק הפרטי של הלקוח | `source='client'` נושא `documentId` בלבד; נפתח דרך Edge Function `portal-open-document` |
| 3 | הודעת מלל בלי שום קובץ | `payload.messageOnly` ⇒ `kind='message'`, כרטיס בלי פקד |

**הפונקציה היחידה שהשתנתה: `build_client_portal`.** שני מקומות בתוכה —
ענף `custom_request` וספירת ההתקדמות. הנוסח נמשך חי מהמסד ורק הם נערכו.

‼ **מה לא נדרש לשינוי, בניגוד למה שנראה מראש:**
`create_onboarding_request` כבר מרשה בקשה בלי דרישות כשהבעלות אינה של הלקוח
(`p_owner='me'`) — וזו הבעלות הנכונה להודעה ממילא, כי היא נסגרת רק כשהמשרד
מסיר אותה. `portal_submit_step` כבר סוגר בקשה כשכל הדרישות סומנו, ולכן
"פתיחת כל הקבצים סוגרת" עובד בלי נגיעה בו.

‼ **הודעה אינה נספרת** ב-`v_done`/`v_total` (`kind <> 'message'`). אין לה
השלמה, ומונה שלעולם לא ייסגר הופך הודעה למטלה פתוחה לנצח.

‼ **תאימות לאחור מלאה:** הענף הישן (`clientResource` יחיד + `clientLinkUrl`
של 114) לא נגע. בקשת מדריך שכבר נשלחה ממשיכה להיפתח, להיסגר ב-`reviewed`,
ולהופיע תחת «מסמכים שימושיים» — מאומת ב-SD-6.

‼ `stepId` נמסר גם על פריט שהושלם: בלעדיו קובץ פרטי היה הופך לבלתי-נגיש
בדיוק ברגע שהבקשה נסגרת ועובר ל«מסמכים שימושיים».

**Edge Function חדשה:** `portal-open-document` (`verify_jwt=false`). פותרת את
טוקן הדף ללקוח, מוודאת שהבקשה שלו ופורסמה, שהמסמך **מופיע ברשימה של אותה
בקשה**, ושהוא שייך לאותו לקוח — ורק אז חותמת קישור ל-5 דקות ומחזירה 302.
הפתיחה נרשמת בדפדפן דרך `portal_submit_step`, כמו קובץ מספריית המשרד.

**שערי רגרסיה:** `scripts/staging-test-send-documents.mjs` (26 בדיקות) ו-
`scripts/staging-test-portal-open-document.mjs` (10 בדיקות אבטחה).

## 145 · אישור עובדה מקצועית — "ריק" אחד, לא שניים (2026-08-31)

**התקלה שתוקנה.** `accept_tax_fact_change` השוותה את תמונת המצב שנשמרה בהצעה
(`old_value.patch`) מול הערך המקובל עכשיו, כדי לא לדרוס בשקט ערך שהשתנה בינתיים.
שני הצדדים ייצגו "אין ערך" בשתי צורות שונות:

| צד | מקור | ייצוג של "ריק" |
|---|---|---|
| הערך בתיק | `_tax_fact_field_op` → `to_jsonb(NULL)` | `NULL` של SQL |
| ההצעה | `old_value.patch` (JSON שמור) | `'null'::jsonb` |

`NULL is distinct from 'null'::jsonb` מחזיר `true` ⇒ **כל שדה שהיה ריק בתיק
נחשב "השתנה מאז ההצעה"** ⇒ `stale_conflict`. אצל לקוח חדש כל השדות ריקים, ולכן
שום ערך מיישור קו לא נכנס לתיק — לא באישור האוטומטי שבסוף המסך, וגם לא בלחיצה
ידנית על "אשר ועדכן בתיק". המנגנון שנועד למנוע דריסה חסם את המסלול התקין כולו.

**התיקון.** `coalesce` על שני הצדדים לפני ההשוואה. ההגנה האמיתית נשמרת: ערך
מקובל שקיים ושונה מהתמונה שבהצעה עדיין מחזיר `stale_conflict` (אומת ב-staging).

‼ **הפונקציה הוגדרה מחדש במלואה** מ-91, עם שינוי בשורת ההשוואה בלבד.

## 146 · "מצב ניכוי במקור" הופך לעובדה מנוהלת (2026-08-31)

עד כה הבחירה במסך יישור הקו (פטור / שיעורים לפי פעילות / אין אישור תקף) נשמרה
ב-`payload` של השלב, ומתוכה נגזר רק `has_exempt_from_withholding`. לכן
**"אין אישור תקף" לא היה ניתן להבחנה מ"שיעורים לפי פעילות"** — בשניהם
`has_exempt_from_withholding=false` — ומחולל הדגלים של "תמונת מצב מול הרשויות"
לא יכול היה לדעת שהאחד תקין והשני דורש טיפול.

- עמודה חדשה `clients.withholding_status` (`exempt` / `rates` / `none`).
- ענף חדש ב-`_tax_fact_field_op`, ו-`withholdingStatus` ב-`GOVERNED_FACT_KEYS`.

‼ **הענף הוזרק בתיקון טקסטואלי ולא בהגדרה מחדש של הפונקציה כולה**: היא כ-20KB
של ענפי `CASE`, והעתקתה ידנית לצורך שלוש שורות מסכנת השמטה שקטה של ענף קיים.
התיקון מאמת את עצמו בשני הכיוונים — האורך גדל בדיוק בגודל התוספת, ומספר הענפים
בדיוק באחד — ואידמפוטנטי (ריצה חוזרת מדלגת). ראה `sql-function-patch-codemod`.

---

## 147 · מסמך שנשלח שומר גם את המלל שצורף לו (2026-08-31)

`supabase/147-sent-document-keeps-its-message.sql` · הוחלה על **staging ופרודקשן**.

תוספת שדה אחת ל-`build_client_portal`: ענף ההשלמה של `custom_request` מחזיר
עכשיו גם `note` (המלל שצורף לקבצים), כפי שהענף הפתוח כבר עשה.

**למה:** בדף האישי הקבצים והמלל חיים באותו מקום — קטע «מסמכים מהמשרד», שהוא
הבית היחיד לכל מה שהמשרד שלח. בלי התוספת ההסבר שהמשרד כתב היה נעלם בדיוק
ברגע שהלקוח פותח את הקובץ האחרון: תוכן שנמחק מול העיניים.

‼ אין שינוי סכימה ואין שינוי התנהגות. כל שאר הפונקציה נמשכה חיה מהמסד ולא נגעה.
מאומת ב-SD-4e.
