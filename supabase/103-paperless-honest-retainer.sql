-- ─── 103: פייפרלס — אין קישור הרשאה שני ──────────────────────────────────────
-- הכרעת גיא (§8): ההרשאה לתשלום קבוע נוצרת בתוך חשבון הפייפרלס של הלקוח, לא
-- דרך קישור ששולחים לו. עד היום retainer_authorization דרש מגיא להדביק
-- authUrl ולשלוח אותו במייל — בדיוק הקישור השני שההחלטה אוסרת.
--
-- מה משתנה כאן (build_client_portal בלבד — שינוי יחיד לעומת מיגרציה 101):
-- ענף retainer_authorization מפסיק להציג כפתור-קישור חיצוני (bucket:'action'
-- + actionKind:'external'). כשההרשאה לא נעולה ולא הושלמה — פריט מידע בלבד
-- (bucket:'office', בלי כפתור), עם המשפט הרגוע: "בפעם הבאה שתיכנס, תתבקש
-- להזין כרטיס". אין ל-UI דרך לדעת אם הכרטיס כבר הוזן בפועל — ההשלמה נשארת
-- ידנית מצד גיא ("הלקוח השלים"), בדיוק כמו היום; לא מומצאת שום יכולת לצפות
-- באירועי פייפרלס שאין לה כיסוי אמיתי.
--
-- payload.authUrl מפסיק להיקרא כאן; payload.authorizationCreatedAt (חדש,
-- נכתב מ-RetainerStepCard דרך advance_onboarding_step('note') הקיים — אין
-- RPC חדש) הוא רק לצריכת גיא במסך שלו, ואינו נוגע בשרת הפורטל בכלל — ההודעה
-- ללקוח זהה לפני ואחרי שגיא יוצר את ההרשאה בפועל.
