-- 51 · קטלוג הבקשות: מסמכים מהלקוח, פרטי הרו"ח הקודם, ניסוח דו-קולי, ושער פרסום
--
-- שלושה דברים שהמנוע לא ידע עד היום:
--   1. לבקש מהלקוח רשימת מסמכים ולספור אותם (2/3).
--   2. לבקש מהלקוח את פרטי הרו"ח הקודם — ומכתב השחרור תלוי בזה.
--   3. להחזיק בקשה מוכנה בלי להראות אותה ללקוח, עד שהרו"ח פותח את התהליך.
--
-- הכול תוספתי. אין מחיקת נתונים, אין שינוי שם, ואין ערך קיים שמשמעותו משתנה.
-- ניסוח דו-קולי: payload.clientTitle / clientSub / clientCta הם קול-הלקוח.
-- אם הם חסרים — הדף האישי נופל לניסוח ברירת מחדל, ולכן שלבים קיימים לא נשברים.
-- שער פרסום: payload.published = false ⇒ הדף האישי מדלג. היעדר השדה = מפורסם.

begin;

-- ── 1. שני סוגי שלב חדשים ─────────────────────────────────────────────────────
...
-- ── 2. המרכיב — מוסיף את שתי הבקשות ומשרשר את מכתב השחרור ────────────────────
