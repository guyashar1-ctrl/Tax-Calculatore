-- ─── 147 · מסמך שנשלח שומר גם את המלל שצורף לו ────────────────────────────
-- קטע «מסמכים מהמשרד» בדף האישי מחזיק את כל מה שהמשרד שלח — שנפתח ושלא —
-- ואת המילים שצורפו לקבצים מעליהם. ענף ההשלמה של custom_request לא החזיר
-- note, ולכן ההסבר נעלם בדיוק כשהלקוח פותח את הקובץ האחרון.
--
-- ‼ תוספת שדה בלבד. אין שינוי סכימה, אין שינוי התנהגות, וכל שאר הפונקציה
-- נמשכה חיה מהמסד (pg_get_functiondef) ולא נגעה.
