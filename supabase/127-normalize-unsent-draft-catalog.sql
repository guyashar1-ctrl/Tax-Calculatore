-- 127 — טיוטה שטרם נשלחה מקבלת את הקטלוג המלא של היום
--
-- ‼ שני חורים שנשארו אחרי 126:
--   1. `excel_ledger` ("כרטסת בפורמט אקסל") הוא מפתח יתום מגרסה ישנה. הכרעת
--      גיא (2026-08-23): הוא **אותו דבר** כמו `ledgers` ("כרטסות הנהלת
--      חשבונות") בקטלוג של היום, ולכן מנרמלים ולא מוסיפים שורה כפולה.
--   2. חלון המכתב מציג אך ורק את מה ששמור בטיוטה (ReleaseLetterDialog:
--      `draft?.materials ?? RELEASE_MATERIALS` — בלי מיזוג). טיוטה שנשמרה
--      לפני שפריט נוסף לקטלוג פשוט לא תציע אותו, לנצח.
--
-- ‼ טיוטות שטרם נשלחו בלבד. בקשה שנשלחה שומרת את `excel_ledger` כפי שהוא:
--    המפתח מקשר מסמכים שהועלו כנגד הפריט, והרו"ח הקודם כבר קיבל את הניסוח.
--
-- ‼ פריט שמתווסף נכנס **לא מסומן**. הקטלוג מציע; המשרד בוחר.

CREATE OR REPLACE FUNCTION public.normalize_unsent_draft_catalog(
  ...
  for r in
    select id, client_id, payload from public.onboarding_steps
     where step_type = 'release_letter'
       and payload->>'releaseSentAt' is null
  ...
  loop
    ...
    for v_item in select x from jsonb_array_elements(r.payload->'releaseDraft'->'materials') x loop
      if v_item->>'key' = 'excel_ledger' then
        if v_hasLed then
          -- כבר יש פריט נוכחי: היתום נבלע, והבחירה שלו לא אובדת.
    ...
    -- ── ב. פריטים מהקטלוג שחסרים בטיוטה ──────────────────────────────────
    for v_cat in select x from jsonb_array_elements(CATALOG) x loop
      if not exists (select 1 from jsonb_array_elements(v_out) y
                      where y->>'key' = v_cat->>'key') then
    ...
    if v_changed then
      ...
      -- ‼ payload בלבד: אין status, אין ball, אין אירוע ואין שליחה.
...
GRANT EXECUTE ON FUNCTION public.normalize_unsent_draft_catalog(boolean) TO service_role;
