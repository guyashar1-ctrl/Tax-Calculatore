...
--  45 · קליטה אינה משימה
-- ═══════════════════════════════════════════════════════════════════════════
--  ‼ "ייצוג מול הרשויות" הופיע פעמיים על המסך: פעם כשלב קליטה ופעם כמשימה
--  "להשלים ייצוג — <שם>" בלוח. אותה עבודה, שתי רשימות, שני מקומות לסמן —
--  ועשרה לקוחות בקליטה היו מייצרים עשר כפילויות.
--
--  שלב הייצוג בקליטה מסונכרן אוטומטית מ-representation_requests (טריגר
--  sync_representation_step, מיגרציה 31c) ולכן הוא כבר מקור האמת. המשימה
--  הייתה עותק ידני שלו.
--
--  ‼ מה שלא נגענו בו: המשימות שכבר קיימות במסד נשארות, והטריגר
--  sync_rep_task_ball (מיגרציה 28) ממשיך לסנכרן אותן עד שייסגרו. השינוי חל
--  על יצירה עתידית בלבד — מחיקת משימות פתוחות של רו"ח היא לא החלטה של מיגרציה.
--
--  ‼ הנוסח כאן הוא העתק מדויק של הפונקציה שרצה במסד (נשלף מ-pg_get_functiondef
--  לפני השינוי), פחות בלוק אחד: ה-insert ל-public.tasks. שום דבר אחר לא זז.
--  v_labels נשאר — הוא עדיין משמש בהערת האירוע על ההצעה.
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- ‼ כאן עמד insert ל-public.tasks שיצר את "להשלים ייצוג — <שם>". הוסר:
  --   שלב הייצוג בקליטה מציג את אותו מידע ומסונכרן מהשרת. ראה כותרת הקובץ.
