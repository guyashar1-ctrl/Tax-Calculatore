-- ─── 99: הדף האישי מראה גם את מה שייפתח בהמשך ────────────────────────────────
-- הכרעת מוצר (2026-08-15, אב-הטיפוס המאושר requests-v2-approved.html):
-- עד היום הדף האישי הראה ללקוח רק את מה שהוא יכול לעשות עכשיו. שלב נעול —
-- "הרשאה לחיוב חודשי" שממתינה לחיבור הפייפרלס — נשלח מהשרת בדלי 'future'
-- ונזרק בדפדפן. התוצאה: הלקוח לא ידע שיש המשך, וכל שלב שנפתח נראה כמו בקשה
-- חדשה שצצה משום מקום.
--
-- מעכשיו: שלב **שפורסם** ונעול מוצג ללקוח בשמו האמיתי, עם נעילה ועם משפט
-- שאומר מה יפתח אותו. זה נותן ללקוח מפת דרכים במקום הפתעות.
--
-- ‼ מה **לא** משתנה — הגבול היחיד שחשוב:
--   טיוטה (published_at is null) עדיין אינה מגיעה ללקוח כלל. הסינון הזה יושב
--   בשאילתת הלולאה (p_mode = 'live') ולא נגעתי בו. נעול ≠ טיוטה:
--     טיוטה         → הלקוח לא רואה בכלל
--     פורסם + נעול  → הלקוח רואה כשלב עתידי, בלי אפשרות לפעול
--     פורסם + פתוח  → הלקוח יכול לפעול
--
-- ‼ תיקון נלווה: 'client_documents' ו-'prev_accountant_details' במצב נעול נפלו
-- עד היום ל-else ויצאו כ-'action' — כלומר הלקוח ראה כפתור פעולה על שלב שהשרת
-- דוחה (portal_submit_step מחזיר 'locked'), ובמקרה של העלאת קובץ אפילו הצליח
-- לפתוח אותו בדלת האחורית (portal-upload-document אינו בודק 'locked').
-- עכשיו שניהם יוצאים כ-'future' — הפער נסגר משני הכיוונים.
--
-- אדיטיבי בלבד: אין טבלה חדשה, אין עמודה, אין שינוי חתימה. build_client_portal
-- נשמר באותה חתימה בדיוק ולכן ההרשאות הקיימות (revoke from public/anon/
-- authenticated — נגיש רק דרך שתי העטיפות) נשארות כפי שהן.

-- ── 1. סיבת הנעילה, בשפה של הלקוח ────────────────────────────────────────────
-- "ייפתח אחרי «פתיחת חשבון פייפרלס»". מרובה-הורים: כל מה שעדיין חוסם, מופרד
-- בנקודה — ורק מה שחוסם. הורה שכבר הושלם יורד מהמשפט, אחרת הלקוח קורא תנאי
-- שכבר התקיים ולא מבין למה השלב עדיין סגור.
--
-- ‼ השם שמוצג הוא הניסוח ללקוח (clientTitle) ולא הכותרת הפנימית. השמות
-- הפנימיים נכתבו בשביל הרו"ח ("מעקב חומרים 2024"), ואסור שידלפו לדף האישי.
...
revoke all on function public.portal_lock_reason(text) from public, anon;
grant execute on function public.portal_lock_reason(text) to authenticated;

-- ── 2. build_client_portal — אותה חתימה, ארבעה שינויים ───────────────────────
-- מקור: 98-portal-tax-status-update-label.sql. השינויים לעומתו:
--   א. client_documents נעול → 'future' (במקום 'action' פעיל שהשרת דוחה)
--   ב. prev_accountant_details נעול → 'future' (אותו תיקון)
--   ג. custom_request נעול → נוסף משפט סיבת הנעילה
--   ד. retainer_authorization נעול → סיבת הנעילה, עם הנפילה לנוסח הקיים
...
begin
  ...
  loop
    ...
    when 'client_documents' then
      ...
      if s.status in ('completed','verified','skipped') then
        ...
        -- ‼ עד 99 נפל ל-else ויצא כ'action': כפתור העלאה על שלב שעדיין נעול.
        v_items := v_items || jsonb_strip_nulls(jsonb_build_object(
    ...
    when 'custom_request' then
      ...
      if s.status in ('completed','verified','skipped') then
        ...
        -- ‼ הסיבה גוברת על הניסוח החופשי: "ייפתח אחרי X" עונה על השאלה
        -- שהלקוח באמת שואל כשהוא רואה שורה נעולה.
        v_items := v_items || jsonb_strip_nulls(jsonb_build_object(
    ...
    when 'prev_accountant_details' then
      if s.status in ('completed','verified','skipped') then
        ...
        -- ‼ אותו תיקון כמו ב-client_documents: טופס פתוח על שלב נעול.
        v_items := v_items || jsonb_strip_nulls(jsonb_build_object(
    ...
    when 'retainer_authorization' then
      if coalesce(s.payload->>'method', '') = 'manual_arrangement' then
        ...
        -- הנוסח הקבוע נשאר כנפילה: מסלול הפייפרלס יוצר את התלות בטריגר,
        -- ויש לקוחות ותיקים שהקשת לא נרשמה אצלם בטבלת התלויות.
  ...
  -- ‼ 'future' נשאר מחוץ ל-total, כמו לפני 99: המונה סופר מה שצריך להיסגר
  -- עכשיו. שלב שעדיין לא נפתח אינו "משימה פתוחה" שהלקוח מפגר אחריה, והכנסתו
  -- ל-total הייתה מזיזה גם את journeyStage (v_done < v_total) לכל לקוח שיש לו
  -- המשך מתוכנן — כלומר משנה מצב מסע בלי שקרה שום דבר.
...
-- החתימה לא השתנתה ולכן ה-ACL נשמר; מוודאים בכל זאת שהפונקציה נשארת
-- מאחורי שתי העטיפות בלבד (get_client_portal / get_client_portal_preview).
