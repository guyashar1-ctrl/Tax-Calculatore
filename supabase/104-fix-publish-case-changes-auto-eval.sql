-- ─── 104: תיקון — publish_case_changes איבדה את הערכת-המחדש האוטומטית ───────
-- ‼ באג שנוצר במיגרציה 101: הרחבתי את publish_case_changes בהתבסס על הגרסה
-- ממיגרציה 81, בלי לשים לב שמיגרציה 83 כבר הוסיפה לה שלב שישי — הערכה-מחדש
-- של משימות אוטומטיות (D3 §5) אחרי חימוש, פלוס autoQueued בתשובה. הגרסה
-- שהוחלה במיגרציה 101 דרסה את זה בטעות וחזרה לגרסת 81. נתפס ע"י
-- staging-test-email-policy.mjs (AT-6), לפני שהגיע לפרודקשן.
--
-- מיגרציה זו היא הגרסה הנכונה: כל מה שנוסף במיגרציה 101 (סידור והסרה
-- ממתינים) פלוס מה שהיה במיגרציה 83 (הערכה אוטומטית + autoQueued), באותו
-- קובץ אחד. שום שינוי נוסף.
