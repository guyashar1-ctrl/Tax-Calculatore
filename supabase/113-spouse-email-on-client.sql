-- ─── 113 · מייל בן/בת הזוג נשמר בכרטיס ──────────────────────────────────────
-- הטופס אוסף את המייל של בן/בת הזוג (לשליחת קישור חתימה אישי), והוא נשמר עד
-- כה על בקשת הייצוג בלבד — ב-identification.spouseEmail וברשומת החותם.
--
-- ‼ הפער: submit_onboarding_full כבר מעדכן בכרטיס את **כל** שאר פרטי בן/בת
-- הזוג — שם, שם פרטי, שם משפחה, שנת לידה ות"ז — וחסר רק המייל, כי לא הייתה
-- לו עמודה. התוצאה הייתה שכתובת שנאספה פעם אחת נקברה בבקשת ייצוג ישנה
-- במקום לשבת בכרטיס לצד השם והת"ז.
--
-- ‼ מה לא נגע: signers וזרימת החתימה. המייל ממשיך להישמר שם בדיוק כפי שהוא
-- היום — זו התוספת של עותק קבוע בכרטיס, לא העברה.

alter table public.clients add column if not exists spouse_email text;

CREATE OR REPLACE FUNCTION public.submit_onboarding_full(p_token text, p_first_name text, p_last_name text, p_id_number text, p_birth_date text, p_secondary_type text, p_secondary_value text, p_phone text, p_email text, p_city text, p_address text, p_family_status text, p_family_status_year integer, p_spouse_name text, p_spouse_email text, p_spouse_id_number text, p_spouse_first_name text DEFAULT NULL::text, p_spouse_last_name text DEFAULT NULL::text, p_spouse_birth_year integer DEFAULT NULL::integer)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- ── מילוי לאחור ─────────────────────────────────────────────────────────────
-- ‼ רק לפי linked_client_id — הקישור המפורש בין הבקשה לכרטיס. בלי ניחוש לפי
-- שם או ת"ז: כתובת מייל שתשויך לאדם הלא נכון גרועה מכתובת חסרה.
-- ‼ רק כשהשדה בכרטיס ריק — ערך שהוזן ידנית גובר על מה שהגיע מהטופס.
update public.clients c
   set spouse_email = x.spouse_email
  from (
    select distinct on (r.linked_client_id)
           r.linked_client_id,
           nullif(trim(coalesce(r.identification->>'spouseEmail', '')), '') as spouse_email
      from public.representation_requests r
     where r.linked_client_id is not null
       and nullif(trim(coalesce(r.identification->>'spouseEmail', '')), '') is not null
     order by r.linked_client_id, r.updated_at desc
  ) x
 where c.id = x.linked_client_id
   and nullif(trim(coalesce(c.spouse_email, '')), '') is null;
