-- ─── 94: M2 — תיקוני נאמנות (בעקבות סקירת Product/UX) ──────────────────────
-- שני נתונים שב-92 נשמרו ל-payload.collected בלבד ("לתיעוד") מוכרזים כאן
-- עובדות מקצועיות מאושרות, בדיוק כמו שאר ה-17 עמודות ב-92: בסיס הכנסה
-- למקדמות בביטוח לאומי, ומצב דיווחים במס הכנסה. אין עריכה במיגרציות
-- 92/93 שכבר הוחלו — זו תוספת בלבד, באותה צורה בדיוק.

-- ── 1. שתי עמודות חדשות ──────────────────────────────────────────────────
alter table public.clients
  add column if not exists ni_income_basis_monthly numeric,
  add column if not exists income_tax_reporting_status text;

-- ── 2. הרחבת ה-allowlist (_tax_fact_field_op) בשני המפתחות ─────────────────
-- ‼ הפונקציה נכתבת מחדש במלואה (כמו בכל הרחבה קודמת) — לא ALTER חלקי,
-- כדי ש-35 הענפים הקיימים יישארו מילה במילה וההרחבה תהיה הבדל של שני
-- ענפים בלבד. revoke/grant בסוף זהים ל-91/92 — allowlist נשאר פנימי בלבד.
create or replace function public._tax_fact_field_op(
  p_client_id text, p_key text, p_write boolean, p_value jsonb, p_source text default null,
  out out_current jsonb, out out_ok boolean
)
returns record
language plpgsql
security definer
...
begin
  ...
  case p_key
    ...
    -- ── 94: שני המפתחות שנוספו בסקירת נאמנות M2 ──────────────────────────
