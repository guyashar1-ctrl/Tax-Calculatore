-- ─── 112 · ספריית מסמכים במקום מדריך יחיד ───────────────────────────────────
-- עד כה היה שדה אחד, settings.expenses_guide, ולכן כל מסמך חדש היה דורש
-- פריט קטלוג משלו. במקומו: settings.client_documents — מערך של מסמכים, כל
-- אחד עם מזהה יציב. בקטלוג יש פריט אחד, «שליחת מסמך ללקוח», ובוחרים בתוכו.
--
-- ‼ תאימות לאחור מלאה: office_document_url קוראת קודם מהספרייה ולפי המזהה,
-- ואם אין — נופלת לשדה הישן. בקשה שכבר נשלחה עם clientResource='expenses_guide'
-- ממשיכה להיפתח בדיוק כמו קודם, בין אם המשרד עבר לספרייה ובין אם לא.
--
-- ‼ הבקשות עצמן לא נגעו. הקישור נפתר בזמן הפתיחה מהגדרות המשרד (כמו קישור
-- הפייפרלס), ולכן החלפת קובץ משנה מיד את מה שייפתח — וגרסאות קודמות
-- נשמרות ב-history ואינן נמחקות מה-Storage.
    ...
    when 'custom_request' then
      ...
      if s.status in ('completed','verified','skipped') then
        v_items := v_items || jsonb_strip_nulls(jsonb_build_object(
          ...
          'resourceUrl', public.office_document_url(p.id, v_res_key)));
      elsif s.status = 'locked' then
        v_items := v_items || jsonb_strip_nulls(jsonb_build_object(
        ...
        v_items := v_items || jsonb_strip_nulls(jsonb_build_object(
          ...
          'kind', case when public.office_document_url(p.id, v_res_key) is not null then 'guide' else 'custom' end,
          'resourceKey', case when public.office_document_url(p.id, v_res_key) is not null then v_res_key else null end,
          'resourceUrl', public.office_document_url(p.id, v_res_key),
