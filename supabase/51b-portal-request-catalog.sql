-- 51b · הדף האישי מכיר את הבקשות החדשות, ומכבד את שער הפרסום
--
-- שלוש תוספות:
--   1. שלב עם payload.published = false אינו מוחזר ללקוח כלל. היעדר השדה = מפורסם,
--      ולכן כל השלבים הקיימים ממשיכים להופיע בדיוק כמו קודם.
--   2. מסמכים מהלקוח ופרטי הרו"ח הקודם מוצגים בקול הלקוח, עם ספירה כשיש צ'קליסט.
--   3. הניסוח נלקח מ-payload.clientTitle/clientSub/clientCta כשקיים, ואחרת
--      מברירת המחדל שהייתה כאן עד היום — כדי שלא יהיה טקסט שנעלם.
--
-- actionKind = 'portal' פירושו: הפעולה נעשית בתוך הדף האישי עצמו, בלי לנווט
-- לטוקן אחר. actionValue הוא מזהה השלב.
