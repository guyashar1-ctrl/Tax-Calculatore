-- 126 — טיוטה שטרם נשלחה מתיישרת לקטלוג הנוכחי
--
-- ‼ פער במיגרציה 123. שם ההתאמה הייתה **לפי ניסוח מדויק** של שני הפריטים
-- המשולבים, וזה נכון לבקשה **שכבר נשלחה**: שכתוב שלה משנה בקשה שמישהו
-- כבר קיבל. טיוטה שאיש לא קיבל אינה נושאת משמעות היסטורית — ולכן שם הכלל
-- היה צר מדי, והמשרד ראה טיוטה עם אוצר מילים ישן בדיוק לפני שליחה.
--
-- כאן ההתאמה היא **לפי מפתח** (uniform_file / pnl_current), ורק כאשר
-- releaseSentAt ריק. מפתח יציב; הניסוח הוא שהשתנה לאורך הגרסאות.
--
-- ‼ לא מוחקים ולא ממזגים כלום. בחירות המשרד נשמרות (checked, priority),
-- ופריטים שאין להם מקבילה בקטלוג של היום — למשל excel_ledger
-- ("כרטסת בפורמט אקסל"), שהוא כרטסת הנהלת חשבונות ולא רווח והפסד —
-- נשארים כפי שהם. מיזוגם היה משנה את מה שהמשרד ביקש.

CREATE OR REPLACE FUNCTION public.align_unsent_release_drafts(
...
GRANT EXECUTE ON FUNCTION public.align_unsent_release_drafts(boolean) TO service_role;
