-- ─── 144 · שליחת מסמכים ללקוח: כמה קבצים, שני מקורות, ומלל חופשי ────────────
-- עד כה בקשת «שליחת מסמך ללקוח» נשאה קובץ אחד, מספריית המשרד בלבד, ונסגרה
-- בהצהרת "עברתי על המדריך". שלוש הגבלות, ושלושתן נופלות כאן:
--
--   1. כמה קבצים בבקשה אחת — payload.clientResources[] במקום clientResource.
--   2. גם קובץ מהתיק הפרטי של הלקוח — source='client' נושא מזהה בלבד, כי
--      client-documents הוא bucket פרטי. הדף האישי פותח אותו דרך
--      portal-open-document, שחותם קישור זמני אחרי שהוא מוודא שהקובץ באמת
--      נשלח ללקוח הזה. קובץ ציבורי מספריית המשרד ממשיך להיפתר ל-URL כאן.
--   3. הודעת מלל בלי שום קובץ — payload.messageOnly. כרטיס שקט לגמרי, בלי
--      פקד, ו**בלי להיספר** ב"כמה נשאר": אין לו השלמה, ומונה שלעולם לא ייסגר
--      הוא בדיוק מה שהופך הודעה למטלה.
--
-- ‼ הפתיחה היא מה שסוגר, ואין יותר הצהרת "עברתי" (הכרעת גיא 2026-08-31).
-- זה חל על בקשות **חדשות** בלבד: בקשה שכבר נשלחה נושאת clientResource יחיד,
-- הענף הישן כאן לא נגע, והיא ממשיכה להיפתח ולהיסגר בדיוק כמו קודם.
--
-- ‼ הנוסח כאן נמשך חי מהמסד (pg_get_functiondef) ב-2026-08-31 ורק שני מקומות
-- שונו — ענף custom_request וספירת ההתקדמות. build_client_portal זהה
-- בפרודקשן ובסטייג'ינג (md5 58236d74…), ולכן אותה מיגרציה חלה על שניהם.
