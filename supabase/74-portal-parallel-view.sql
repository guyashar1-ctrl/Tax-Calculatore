-- 74 — הדף האישי: ההצעה והייצוג כשלבים עם מצב, לא כשורות בתחתית.
--
-- הדף עצמו מאחד את "בטיפול המשרד" ו"הושלם" למקטע אחד ("במקביל"), ולכן הניסוח
-- כאן צריך לעמוד בפני עצמו: כל שורה אומרת גם מה כבר נסגר וגם מה קורה עכשיו.
--   "אישור הצעת המחיר"  → "הצעת המחיר אושרה"   (אבן דרך, לא מטלה)
--   "ייפוי הכוח בבדיקה" → "ייפוי הכוח" + "נחתם על ידך — עכשיו בבדיקה אצלנו"
--
-- ‼ שינוי ניסוח בלבד. המבנה, הדליים והלוגיקה זהים ל-live-2026-08-07.
    ...
    when 'representation' then
      if req.id is null then
        ...
        if v_sign_token is not null then
          ...
          v_items := v_items || jsonb_build_object('bucket','office','key','rep_sign_spouse','label','ייפוי הכוח','sub','ממתינים לחתימת בן/בת הזוג — קישור אישי נשלח במייל נפרד');
        end if;
      elsif req.status in ('awaiting_accountant', 'awaiting_stamp') then
        v_items := v_items || jsonb_build_object('bucket','office','key','rep_office','label','ייפוי הכוח','sub','נחתם על ידך — עכשיו בבדיקה ובחתימה אצלנו');
      elsif req.status = 'awaiting_authorities' then
        v_items := v_items || jsonb_build_object('bucket','office','key','rep_authorities','label','ייפוי הכוח','sub','נחתם והוגש לרשויות המס — ממתינים לאישור');
