...
function job(p: { id: string; actionType: string; status: AutomationJobStatus; ageMin: number; errorCode?: string }): AutomationJob {
  ...
  return {
    ...
    errorCode: p.errorCode, createdAt, updatedAt: createdAt,
  };
}
...
test('C. explicit click must cancel an old open needs_human/queued job before creating a new one', () => {
  ...
  assert.equal(mustCancelBeforeStart(oldNeedsHuman), true);
  assert.equal(mustCancelBeforeStart(oldQueued), true);
  // running אינה ניתנת לביטול ב-cancel_automation_job — העובד תופס אותה מחדש.
  assert.equal(mustCancelBeforeStart(job({ id: 'r', actionType: SHAAM, status: 'running', ageMin: 60 })), false);
  assert.equal(mustCancelBeforeStart(job({ id: 'f', actionType: SHAAM, status: 'failed', ageMin: 1 })), false);
  assert.equal(mustCancelBeforeStart(null), false);
});
...
test('F. BTL path: independent selection, connecting/needs_you/ready derive the same way', () => {
  ...
  assert.equal(mustCancelBeforeStart(btlNeeds), true);
});
...
test('G. the diagnosed trap: hidden old open job → created:false → hidden again — cannot recur', () => {
  ...
  assert.equal(mustCancelBeforeStart(loaded), true, 'and the click will cancel it before creating');
